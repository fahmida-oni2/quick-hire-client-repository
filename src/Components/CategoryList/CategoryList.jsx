        ...
        {categories.map((cat, index) => {
          ...
          return (
            <button
              ...
              className={`flex sm:flex-col flex-row items-center sm:items-start text-left p-4 sm:p-6  border transition-all duration-200 cursor-pointer group gap-4 sm:gap-0 ${
                isActive
            ...
            >
              <div
                className={`shrink-0 ${isActive ? "text-white" : "text-primary"}`}
              >
                {cat.icon}
              </div>
              <div className="sm:mt-4">
                <p
                  className={`font-bold text-sm sm:text-base mb-0.5 sm:mb-1 ${isActive ? "text-white" : "text-[#25324B]"}`}
                >
                  {cat.name}
                </p>
                <div
                  className={`flex justify-between items-center text-xs sm:text-sm ${isActive ? "text-white/80" : "text-gray-400"}`}
                >
                  <div>{cat.jobs} jobs available</div>
                  <div>
                    <FiArrowRight
                      size={12}
                      className={`transition-transform ml-50 lg:ml-5 md:ml-0 group-hover:translate-x-1 ${isActive ? "text-white" : "text-primary"}`}
                    />
                  </div>
                </div>
              </div>
